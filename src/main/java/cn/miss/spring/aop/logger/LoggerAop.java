package cn.miss.spring.aop.logger;
...
public class LoggerAop {
    ...
    @Pointcut(value = "@annotation(cn.miss.spring.aop.logger.anno.Logger)")
    public void methodPoint() {
    }

    @Pointcut("@within(cn.miss.spring.aop.logger.anno.Logger)")
    ...
    private void printAfter(ProceedingJoinPoint proceedingJoinPoint, Object result) {
        try {
            ...
            final Method method = methodSignature.getMethod();
            final String className = proceedingJoinPoint.getTarget().getClass().getSimpleName();
            logger.info(className + "." + method.getName() + ": returnValue: " + result);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
